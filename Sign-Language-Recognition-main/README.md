...
Browser: MediaPipe Holistic (via CDN) in web/islr to detect landmarks and draw overlays.
API (ISLR): FastAPI + Uvicorn in main.py, serving the web page and a /islr/predict endpoint.
ML Inference: TensorFlow Lite in model.py with model.tflite and dict_sign.csv class map.
Frontend (banking UI): React in frontend.
Backend (banking/auth): Node.js/Express in backend (JWT, PostgreSQL, Razorpay, email, file upload).
Face/Voice microservice: Flask in app.py (InsightFace + SpeechBrain